...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=+5569992419378:+55 (69) 9241-9378\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
  ...
  function pad(s){
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oie gostosa@${num.split('@')[0]}\nBem vindo (𝐀) ao grupo *${mdata.subject}* leia as regras pra não ser removido 🤙🤪 se sair tem recadinho carinhoso😈🤙`
				...
				teks = `vai,saiaa de nosso grupokksjkk FDP cheira a pika vsfkkjksk vai iii se volta manda foto do cú pprt ghost do krlh CAGUEI@${num.split('@')[0]}VASA`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'FICA FRIO AI,TO DESINROLANDO AQUI😋🙈',
				success: '✔️Sucesso✔️',
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nSLV mano como!\nVocê nn está registrado no banco de dados do japa tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar japa|10`,
				}
			}
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'novo macaco na administração :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem?', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto imundo', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MARCANDO TODOS OS RANDOM🙈〙✪══\n╠➥'+teks+'╚═〘 ARR157 BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 MARCANDO TODOS OS RANDOM🙈〙✪══\n╠➥'+teks+'╚═〘 ARR157 BOT 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					reply('╔══✪〘 MARCANDO TODOS OS RANDOM🙈 〙✪══\n╠➥'+teks+'╚═〘 ARR157 BOT 〙')
					break
				case 'send':
					var pc = body.slice(6)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'filho da putas me iludiu,tristão vasei... som de peido pro ses ooooh (pomm pomm)acho q alguém peidei🤙🤪) ', text) // ur cods
					}, 0)
                     break
	    ...
	    case 'kapankah':
            ...
            break
						...
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'capetinha roxo😈'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwbobs': 
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'fofo'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				case 'clone':
					...
					break
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter JAPA SULISTA`)
                                        if (!isUser) return reply(mess.only.daftarB)
